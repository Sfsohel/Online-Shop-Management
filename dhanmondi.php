...
<?php
   ...
   $total="select * from product where place='Dhanmondi' and validity='accepted' ";
   ...
   if(isset($_GET['page'])&& !empty($_GET['page']))
   {
	...
	$sql = "select * from product where place='Dhanmondi' and validity='accepted' limit $item_per_page offset $offset";
		...
		while($row = $result->fetch_assoc()) {
		 ...
		 for($i=1;$i<=$totalpages;$i++){
			 ...
			 echo '<a class="active">'.$i.'</a>';
					 ...
					 echo '<a href="dhanmondi.php?page='.$i.'">'.$i.'</a>';
